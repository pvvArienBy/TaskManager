...
public class UserController {
    ...
    public UserDTO findById(@PathVariable UUID uuid) {
        return conversionService.convert(userService.findById(uuid), UserDTO.class);
    }
    ...
    public ResponseEntity<UserDTO> save(@PathVariable UUID uuid, @PathVariable  Long dt_update, @RequestBody UserCreateDTO userCreateDTO) {
        UserEntity userEntity = userService.save(uuid, dt_update, userCreateDTO);
        UserDTO userDTO = conversionService.convert(userEntity, UserDTO.class);
        return ResponseEntity.status(HttpStatus.CREATED).body(userDTO);
    }
